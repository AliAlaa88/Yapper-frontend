...
describe('TweetsList component', () => {
    ...
    describe('Multiple Pages', () => {
        it('renders tweets from multiple pages', async () => {
            const module = await import('../../queries/useTweetQueries')
            vi.spyOn(module, 'useTweetsQuery').mockImplementation(
                () =>
                    ({
                        data: ref({
                            pages: [
                                {
                                    data: [{ tweet_id: 't1', user: { username: 'alice' } }],
                                    nextCursor: 'page2',
                                },
                                {
                                    data: [{ tweet_id: 't2', user: { username: 'bob' } }],
                                    nextCursor: null,
                                },
                            ],
                        }),
    ...
    describe('Loading States', () => {
        it('provides hasNextPage state', async () => {
            const module = await import('../../queries/useTweetQueries')
            vi.spyOn(module, 'useTweetsQuery').mockImplementation(
                () =>
                    ({
                        data: ref({
                            pages: [{ data: [{ tweet_id: 't1', user: { username: 'alice' } }] }],
                        }),
                        isLoading: ref(false),
                        error: ref(null),
                        refetch: vi.fn(),
            ...
            const wrapper = mount(TweetsList as any, {
                global: { stubs: { Tweet: true }, mocks: { $t: (msg) => msg } },
            })

            expect(wrapper.exists()).toBe(true)
        })

        it('handles isFetchingNextPage state', async () => {
            const module = await import('../../queries/useTweetQueries')
            vi.spyOn(module, 'useTweetsQuery').mockImplementation(
                () =>
                    ({
