...
export function useVerifyForgotPasswordOTPQuery(
    ...
    return useMutation({
        ...
        mutationFn: ({ identifier, token }: { identifier: string; token: string }) =>
            $authService.verifyForgotPasswordOTP(identifier, token),
        retry: false,
        onSuccess: (data) => onSuccess?.(data),
        onError: (error) => onError?.(error),
    })
...
export function useResetPasswordQuery(
    ...
    return useMutation({
        ...
        mutationFn: ({
            ...
            newPassword: string
            reset_token: string
        }) => $authService.resetPassword(identifier, newPassword, reset_token),
        retry: false,
        onSuccess: (data) => onSuccess?.(data),
        onError: (error) => onError?.(error),
    })
