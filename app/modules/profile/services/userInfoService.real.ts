...
export const userInfoServiceReal = {
    ...
    async getFollowers(userId: string, mutual: boolean, cursor?: string): Promise<FollowUsersPage> {
        const { $axios } = useNuxtApp()
        try {
            const response = await $axios.get(
    ...
    async getFollowing(userId: string, cursor?: string): Promise<FollowUsersPage> {
        const { $axios } = useNuxtApp()
        try {
            const response = await $axios.get(
